...
if (diff_soft=="DESeq2"){  
  ...
  for (i in 1:dim(combn(unique(d),2))[2]){
    ...
    write.table(table,
            file=paste0(path,"/final_results_reanalysis/DGE/DGE_analysis_DESEQ2_comp",combn(unique(d),2)[,i][1],"vs",combn(unique(d),2)[,i][2],".txt"),quote = F,row.names = F, col.names = T,sep = "\t")
  }
}
print("ALL DONE")
