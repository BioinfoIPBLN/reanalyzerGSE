...
## CREDITS: https://github.com/PoisonAlien/beadAnalyze
## ADAPTER for use in GSE-Reanalyzer by JLRUIZ IPBLN-CSIC 2022.
...
beadAnalyze = function(idats, names, condition, ref.condition, fdr = NULL, plotPCA = F){
  
  bead.batch = readIdatFiles(idatFiles = idats) #read idat files
  names <- paste(pData(bead.batch)[,1],names,sep="_")
  names_2 <- pData(bead.batch)[,1]
  names_eq <- data.frame(names_1=names,names_2=names_2)
  ...
  sampleNames(assayData(bead.batch)) <- names
  sampleNames(protocolData(bead.batch)) <- names
  ...
  print("I have a valid GSE object:")
  print(validObject(bead.batch))
  print(identical(sampleNames(bead.batch),sampleNames(assayData(bead.batch))))
  print(identical(sampleNames(bead.batch),sampleNames(phenoData(bead.batch))))
  print(identical(sampleNames(bead.batch),sampleNames(protocolData(bead.batch))))
  ...
  if(length(pData(eset)[,1]) == 2){
    ...
    # return(exprs.df)
    return(list(eset = eset, expr_df = exprs.df, names_eq = names_eq))
  } else if (!is.null(fdr)){
    ...
    if(plotPCA){
      p <- pca_plot(mat = res[,names], pdata = pData(eset), label = T)  
    }
    
    # return(list(results = res, eset = eset))  
    # return(res)
    return(list(results = res, eset = eset, expr_df = exprs.df, pca = p, names_eq = names_eq))      
  }
  else {
  cat('returning just expression table')
  return(list(eset = eset, expr_df = exprs.df, names_eq = names_eq))}
}
