...
a <- lapply(a,function(x){x[,sort(colnames(x))]})
b <- lapply(a,colnames)
if (length(unique(b)) == 1){
	d <- bind_rows(a, .id = "column_label")
	d <- as.data.frame(d[order(d[,names(which.min(nchar(grep("^GSM",d[1,],val=T))))[1]]),]) # Ensure the order of the metadata is following the GSMXXXX ids and the SRR ids, because this is the order in the series_matrix tables from which I get more info
