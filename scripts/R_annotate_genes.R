...
if(organism_cp=="Homo sapiens"){
  ...
  suppressMessages(library("org.Mm.eg.db",quiet = T,warn.conflicts = F))
	...
	print("For now the only organism supported for annotating gene IDs are human and mouse. There are plans to add more in the future, including the already available orgDB and building custom databases")
	stop("Exiting...")
}
...
for (file in grep("annotation",list.files(path=path,recursive=T, include.dirs=T, full.names=T,pattern=pattern_to_match),invert=T,val=T)){
	a <- read.delim(file); b <- merge(a,annot_summary_final,by.x="Gene_ID",by.y="SYMBOL",all.x=T)
