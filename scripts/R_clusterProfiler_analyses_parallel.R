  ...
  process_file_within <- function(name_internal){
    ...
    ###### 2. GO over-representation analyses:
      ...
      tryCatch({
                  ...
                  ego_df$summary_LogFC <- unlist(lapply(strsplit(ego_df$geneID,"/"),function(x){paste(x,a$logFC_sense[a$Gene_ID %in% x[order(x)]],sep="_",collapse="/")}))
      ...
      ###### 3. KEGG over-representation:    
          ...
          tryCatch({
              ...
              kk_write$summary_LogFC <- unlist(lapply(strsplit(kk_write$Gene_ID,","),function(x){paste(x,a$logFC_sense[a$Gene_ID %in% x[order(x)]],sep="_",collapse="/")}))              
      ...
      ###### 5. Reactome over-representation:      
          ...
          tryCatch({
            ...
            df$summary_LogFC <- unlist(lapply(strsplit(df$Gene_ID,","),function(x){paste(x,a$logFC_sense[a$Gene_ID %in% x[order(x)]],sep="_",collapse="/")}))
      ...
      ###### 7. Gene Set Enrichment Analysis of Gene Ontology:
          ...
          if(geneset=="fdr_05" || geneset=="fdr_01"){
              ...
              tryCatch({
                        ...
                        df$summary_LogFC_2 <- unlist(lapply(strsplit(df$summary_LogFC,",",),function(x){paste(names(table(x)),table(x),sep="_",collapse=",")}))
                        df$summary_LogFC <- unlist(lapply(strsplit(df$core_enrichment,"/"),function(x){paste(x,a$logFC_sense[a$Gene_ID %in% x[order(x)]],sep="_",collapse="/")}))
                        df$summary_POS <- unlist(lapply(strsplit(df$summary_LogFC,"/"),function(x){paste(sort(unique(gsub("_POS","",grep("_POS",x,val=T)))),collapse=",")}))
                        df$summary_NEG <- unlist(lapply(strsplit(df$summary_LogFC,"/"),function(x){paste(sort(unique(gsub("_NEG","",grep("_NEG",x,val=T)))),collapse=",")}))
