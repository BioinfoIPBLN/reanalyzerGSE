...
sub run_miARma{
	...
	if($cfg->SectionExists("Aligner")==1){
		...
		if($cfg->exists("Aligner","aligner") eq "" or ($cfg->val("Aligner","aligner") eq "")){
		...
		elsif( lc($cfg->val("Aligner","aligner")) eq "star" and $cfg->exists("Aligner","starindex") ne "" and ( $cfg->val("Aligner","gtf") eq "" )){
			...
			if(scalar(@files)>0){
				...
				# Reading the array with the names of the files
				# JLR: The call to STAR is whithin the loop foreach, but because of my modification of Aligner.pm introducing GNU's parallel only the first iteration is effective, the rest won't execute STAR because I check if the output files exists before executing
				my $count=0;
				foreach my $file(sort @files){				
					# Always stop after first iteration...
					print $count++."... Proceeding\n";
			...
			if(lc($cfg->val("General","type")) ne "circrna"){
				...
				if(scalar(@files)>0 and lc($cfg->val("General","type")) ne "circrna"){
					...
					foreach my $file(@files){
						...
						# JLR: The call to featureCounts is whithin the loop foreach, but because of my modification of Readcount.pm introducing GNU's parallel only the first iteration is effective, the rest won't execute featureCounts because I check if the output files exists before executing
