...
sub IndexGeneration{
	...
	my $starindex=$args{"starindex"};
	my $hisat2index=$args{"hisat2index"};
...
sub bowtie1{
	...
	#First, check that bowtie is in path:
	my @bowtie1_bin=`which bowtie`;
	#Executing the command
	if(scalar(@bowtie1_bin)<1){
...
sub hisat2_index{
	...
	#Check starindex_final
	if($hisat2index ne ""){
		$hisat2idx_final=$hisat2index;
	}		
	else{
		$hisat2idx_final=$indexdir.$indexname."/".$indexname;
	}
	
	#Checking the mandatory arguments
	if ($fasta and $dir and $indexname and $logfile){
		print STDERR date()." Generating the index genome $indexname from $fasta. This process could take some hours and need much RAM memory, particularly when multithreading. If segfaults in the alignment step, please do check the log and warnings, it may be required to rerun the indexing manually changing --genomeSAindexNbases...\n";
		#bowtie-build execution command from a fasta file. The output index will be saved
		#in the genomeindex1 directory with the name index 
		my $command="hisat2-build -p $indexthreads -f ".$fasta." ".$hisat2idx_final;
		#commandef is the command that will be executed by system composed of the index
		#directory creation, the module loading, and the bowtie-build execution. The error 
		#data will be redirected to the run.log file
		$commanddef="mkdir -p \$(dirname ".$hisat2idx_final."); ".$command ." >> ".$logfile." 2>&1";
		#Printing the date and command execution on the run.log file
...
sub hisat2{
	...
	my $file=$args{"file"}; #File which is going to be aligned
	my $hisat2index=$args{"hisat2index"}; #Genome index in format
	my $indexname=$args{"indexname"};
	my $indexdir=$args{"indexdir"};
	my $threads=$args{"threads"}; #Optional number of threads to perform the analysis faster
	my $logfile=$args{"logfile"}; #Path of the logfile to write the execution data
	...
	my $hisatpardef;
	my $hisat2idx_final;
	#Check hisat2index_final
	if(defined($args{"hisat2index"})){
		$hisat2idx_final=$hisat2index;
	}		
	else{
		$hisat2idx_final=$indexdir.$indexname."/".$indexname;
	...
	#Number of threads can be provided by the user
	if($threads>0){
		$hisatpardef.= " -p " . ($threads + 1);
	}

	#Checking the mandatory parameters
	if ($file and $projectdir and $hisat2idx_final and $logfile and $statsfile){ 
		#Extracting the name of the file
		my $name=fileparse($file, qr{\.f.*});
		my $output_file_bw2;
		if($adapter){
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
			...
			if($file =~ /.*_R?1.*/){
				...
				if(-e $mate_file){
					if($file ne $mate_file){						
						$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && parallel --verbose --joblog ".$projectdir."/hisat2_log_parallel.txt -j ".$parallelnumber." 'hisat2 -q -t --seed 123 --very-sensitive ".$hisatpardef." -x ".$hisat2idx_final." -1 \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_1.fastq.gz -2 \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_2.fastq.gz --met-file {}.metrics --un-conc-gz {}_no_aligned.fastq.gz | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_.bam##idx##{}_nat_str_.bam.bai - && echo {}_nat_str_.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_.bam --java-mem-size=".$memorylimit_div." &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_.bam -gtf ".$gtf." -pe -outdir \$PWD/rnaseqqc_results/{}_nat_str_.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_.bam -o {}_nat_str_.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBigwigSummary bins -p ".$indexthreads." --bwfiles \$( ls | egrep '.bw\$' ) -out deeptools_all_bws.npz && plotCorrelation --corData deeptools_all_bws.npz --corMethod spearman --plotFile deeptools_all_bws.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bws.npz --corMethod pearson --plotFile deeptools_all_bws.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA.pdf --outFileNameData deeptools_all_bws.npz_PCA.tab && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_2.pdf --outFileNameData deeptools_all_bws.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_3.pdf --outFileNameData deeptools_all_bws.npz_PCA_3.tab --transpose; fi";
					}
				}
				else{
					print STDERR "ERROR:: You have requested a Paired-End analysis, so for the file $file a $mate_file file is needed\n";
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && parallel --verbose --joblog ".$projectdir."/hisat2_log_parallel.txt -j ".$parallelnumber." 'hisat2 -q -t --seed 123 --very-sensitive ".$hisatpardef." -x ".$hisat2idx_final." -U \$(cat ".$tmp_file." | xargs dirname | uniq)/{} --met-file {}.metrics --un-conc-gz {}_no_aligned.fastq.gz | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_.bam##idx##{}_nat_str_.bam.bai - && echo {}_nat_str_.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_.bam --java-mem-size=".$memorylimit_div." &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_.bam -gtf ".$gtf." -outdir \$PWD/rnaseqqc_results/{}_nat_str_.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_.bam -o {}_nat_str_.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log' ::: \$(cat ".$tmp_file." | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBigwigSummary bins -p ".$indexthreads." --bwfiles \$( ls | egrep '.bw\$' ) -out deeptools_all_bws.npz && plotCorrelation --corData deeptools_all_bws.npz --corMethod spearman --plotFile deeptools_all_bws.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bws.npz --corMethod pearson --plotFile deeptools_all_bws.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA.pdf --outFileNameData deeptools_all_bws.npz_PCA.tab && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_2.pdf --outFileNameData deeptools_all_bws.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_3.pdf --outFileNameData deeptools_all_bws.npz_PCA_3.tab --transpose; fi";
		}
	...
	else
	{
		#Registering the error
   		open(LOG,">> ".$logfile) || die "HISAT2 ERROR :: Can't open $logfile: $!";
    	print LOG "HISAT2 :: ".date()." File($file), logfile($logfile), projectdir ($projectdir), statsfile($statsfile) and/or index($hisat2idx_final) have not been provided";
    	close LOG;

		#If mandatory parameters have not been provided program dies and shows error message
		warn ("HISAT2 :: ".date()." File($file), logfile($logfile), projectdir ($projectdir), statsfile($statsfile) and/or index($hisat2idx_final) have not been provided");
		help_hisat2();
	}
	sub help_hisat2{
	    my $usage = qq{
...
sub star_index{
	...
	#Check starindex_final
	if($starindex ne ""){
		$staridx_final=$starindex;
	}		
	else{
		$staridx_final=$indexdir.$indexname."/";
	}

	#Checking the mandatory arguments
	if ($fasta and $dir and $gtf and $logfile){
		print STDERR date()." Generating the index genome from $fasta. This process could take some hours and need much RAM memory, particularly when multithreading. If segfaults in the alignment step, please do check the log and warnings, it may be required to rerun the indexing manually changing --genomeSAindexNbases and the recommended value will be there...\n";
		#bowtie-build execution command from a fasta file. The output index will be saved
		#in the genomeindex1 directory with the name index 
		my $command="STAR --runThreadN ".$indexthreads." --runMode genomeGenerate --genomeDir ".$staridx_final." --genomeFastaFiles $fasta --sjdbGTFfile $gtf --sjdbOverhang $readlength --limitGenomeGenerateRAM $memorylimit";
		#commandef is the command that will be executed by system composed of the index
		#directory creation, the module loading, and the bowtie-build execution. The error 
		#data will be redirected to the run.log file
...
sub star{
	...
	my $file=$args{"file"}; #File which is going to be aligned
	my $starindex=$args{"starindex"} || undef; #Genome index in format	
	my $indexname=$args{"indexname"};
	my $indexdir=$args{"indexdir"};	
	my $threads=$args{"threads"}; #Optional number of threads to perform the analysis faster
	my $logfile=$args{"logfile"}; #Path of the logfile to write the execution data
	...
	#Number of threads can be provided by the user
	if($threads>0){
		$starpardef.= " --runThreadN " . ($threads + 1);
	}
	...
	if ($file and $projectdir and $logfile and $statsfile){ 
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
			...
			if($file =~ /.*_R?1.*/){
				...
				if(-e $mate_file){
					if($file ne $mate_file){
						...
						$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && STAR --runThreadN ".$indexthreads." --genomeDir ".$staridx_final." --genomeLoad LoadAndExit --outFileNamePrefix ".$projectdir.$output_dir."genomeloading.tmp2 && rm -rf ".$projectdir.$output_dir."genomeloading.tmp* && parallel --verbose --joblog ".$projectdir."/star_log_parallel.txt -j ".$parallelnumber." 'STAR --runMode alignReads --genomeDir ".$staridx_final." --genomeLoad LoadAndKeep --readFilesIn \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_1.fastq.gz \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_2.fastq.gz --outFileNamePrefix ".$projectdir.$output_dir."{}_nat_str_ $starpardef --outStd SAM | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_Aligned.out.bam##idx##{}_nat_str_Aligned.out.bam.bai - && echo {}_nat_str_Aligned.out.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_Aligned.out.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_Aligned.out.bam --java-mem-size=".$memorylimit_div." &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_Aligned.out.bam -gtf ".$gtf." -pe -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.out.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_Aligned.out.bam -o {}_nat_str_Aligned.out.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBigwigSummary bins -p ".$indexthreads." --bwfiles \$( ls | egrep '.bw\$' ) -out deeptools_all_bws.npz && plotCorrelation --corData deeptools_all_bws.npz --corMethod spearman --plotFile deeptools_all_bws.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bws.npz --corMethod pearson --plotFile deeptools_all_bws.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA.pdf --outFileNameData deeptools_all_bws.npz_PCA.tab && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_2.pdf --outFileNameData deeptools_all_bws.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_3.pdf --outFileNameData deeptools_all_bws.npz_PCA_3.tab --transpose; fi";
					}
				}
				else{
					print STDERR "ERROR:: You have requested a Paired-End analysis, so for the file $file a $mate_file file is needed\n";
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && STAR --runThreadN ".$indexthreads." --genomeDir ".$staridx_final." --genomeLoad LoadAndExit --outFileNamePrefix ".$projectdir.$output_dir."genomeloading.tmp2 && rm -rf ".$projectdir.$output_dir."genomeloading.tmp* && parallel --verbose --joblog ".$projectdir."/star_log_parallel.txt -j ".$parallelnumber." 'STAR --runMode alignReads --genomeDir ".$staridx_final." --genomeLoad LoadAndKeep --readFilesIn \$(cat ".$tmp_file." | xargs dirname | uniq)/{} --outFileNamePrefix ".$projectdir.$output_dir."{}_nat_str_ $starpardef --outStd SAM | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_Aligned.out.bam##idx##{}_nat_str_Aligned.out.bam.bai - && echo {}_nat_str_Aligned.out.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_Aligned.out.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_Aligned.out.bam --java-mem-size=".$memorylimit_div." &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_Aligned.out.bam -gtf ".$gtf." -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.out.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_Aligned.out.bam -o {}_nat_str_Aligned.out.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log' ::: \$(cat ".$tmp_file." | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBigwigSummary bins -p ".$indexthreads." --bwfiles \$( ls | egrep '.bw\$' ) -out deeptools_all_bws.npz && plotCorrelation --corData deeptools_all_bws.npz --corMethod spearman --plotFile deeptools_all_bws.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bws.npz --corMethod pearson --plotFile deeptools_all_bws.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bws.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA.pdf --outFileNameData deeptools_all_bws.npz_PCA.tab && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_2.pdf --outFileNameData deeptools_all_bws.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bws.npz --plotFile deeptools_all_bws.npz_PCA_3.pdf --outFileNameData deeptools_all_bws.npz_PCA_3.tab --transpose; fi";
		}
...
sub ReadSummary{
	...
	if(lc($aligner) eq "star" ){
		...
		# mkdir($summary_path->{$aligner});
		opendir ( DIR, $summary_path->{$aligner} ) || die "Error in opening the dir ".$summary_path->{$aligner} ."\n";
