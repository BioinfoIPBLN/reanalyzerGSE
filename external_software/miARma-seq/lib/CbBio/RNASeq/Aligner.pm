...
sub hisat2{
	...
	if ($file and $projectdir and $hisat2idx_final and $logfile and $statsfile){ 
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
			...
			my $memorylimit_div_mb_sort_cores = int($memorylimit_div_mb / ($threads - 1));
	...
	if ($file and $projectdir and $logfile and $statsfile){ 
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
				...
				if(-e $mate_file){
					if($file ne $mate_file){
						...
						$command=qq{if [ \$(ls $projectdir$output_dir | wc -l) -eq 0 ]; then
					                          ...
					                          parallel --verbose --joblog ${projectdir}/star_log_parallel.txt -j $parallelnumber 'STAR --runMode alignReads --genomeDir $staridx_final --genomeLoad LoadAndKeep --readFilesIn \$(cat $tmp_file | xargs dirname | uniq)/{}_1.fastq.gz \$(cat $tmp_file | xargs dirname | uniq)/{}_2.fastq.gz --outFileNamePrefix $projectdir${output_dir}{}_nat_str_ $starpardef --outStd SAM | samtools sort -@ $threads -l 9 -m ${memorylimit_div_mb_sort_cores}M --write-index -o {}_nat_str_Aligned.out.bam##idx##{}_nat_str_Aligned.out.bam.bai -' ::: \$(echo \$name_lists)
					                          ...
					                          qualimap rnaseq -bam $projectdir${output_dir}/{}_nat_str_Aligned.out.bam -gtf $gtf -pe -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.out.bam --java-mem-size="${memorylimit_div_mb}m" || true
					                          ...
					                          export _JAVA_OPTIONS="-Xms5g -Xmx${memorylimit_in_mb}m -Djava.io.tmpdir=\$PWD" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ || true
			...
			$command=qq{if [ \$(ls $projectdir$output_dir | wc -l) -eq 0 ]; then
		                          ...
		                          STAR --runThreadN $indexthreads --genomeDir $staridx_final --genomeLoad LoadAndExit --outFileNamePrefix $projectdir${output_dir}genomeloading.tmp2 && rm -rf $projectdir${output_dir}genomeloading.tmp*
		                          parallel --verbose --joblog ${projectdir}/star_log_parallel.txt -j $parallelnumber 'STAR --runMode alignReads --genomeDir $staridx_final --genomeLoad LoadAndKeep --readFilesIn \$(cat $tmp_file | xargs dirname | uniq)/{} --outFileNamePrefix $projectdir${output_dir}{}_nat_str_ $starpardef --outStd SAM | samtools sort -@ $threads -l 9 -m ${memorylimit_div_mb_sort_cores}M --write-index -o {}_nat_str_Aligned.out.bam##idx##{}_nat_str_Aligned.out.bam.bai -' ::: \$(echo \$name_lists)
		                          parallel --verbose --joblog ${projectdir}/starprocess_log_parallel.txt -j $parallelnumber 'export _JAVA_OPTIONS="-Xms5g -Xmx${memorylimit_div_mb}m -Djava.io.tmpdir=\$PWD" && qualimap bamqc -bam $projectdir${output_dir}/{}_nat_str_Aligned.out.bam -nt $threads -gff $gtf -c -outdir \$PWD/bamqc_results/{}_nat_str_Aligned.out.bam --java-mem-size="${memorylimit_div_mb}m" || true
		                          qualimap rnaseq -bam $projectdir${output_dir}/{}_nat_str_Aligned.out.bam -gtf $gtf -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.out.bam --java-mem-size="${memorylimit_div_mb}m" || true
		                          bamCoverage -b {}_nat_str_Aligned.out.bam -o {}_nat_str_Aligned.out.bam.bw -of bigwig -bs 10 -p $threads --normalizeUsing RPKM' ::: \$(echo \$name_lists)
		                          for f in \$( ls | egrep '.bam\$' ); do echo \$f"\t"\$PWD/bamqc_results/\$f >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j $parallelnumber 'samtools flagstat -@ $threads {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ $threads {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' )
		                          export _JAVA_OPTIONS="-Xms5g -Xmx${memorylimit_in_mb}m -Djava.io.tmpdir=\$PWD" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ || true
		                          multiBamSummary bins -p $indexthreads --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz
