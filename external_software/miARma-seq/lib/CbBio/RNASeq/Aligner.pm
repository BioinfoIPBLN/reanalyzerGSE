	...
	if ($file and $projectdir and $hisat2idx_final and $logfile and $statsfile){ 
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
			...
			if($file =~ /.*_R?1\.f.*/){
				...
				if(-e $mate_file){
					if($file ne $mate_file){						
						$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && parallel --verbose --joblog ".$projectdir."/hisat2_log_parallel.txt -j ".$parallelnumber." 'echo {} && hisat2 -q -t --seed 123 --very-sensitive ".$hisatpardef." -x ".$hisat2idx_final." -1 \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_1.fastq.gz -2 \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_2.fastq.gz --met-file {}.metrics --un-conc-gz {}_no_aligned.fastq.gz | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_.bam##idx##{}_nat_str_.bam.bai - && echo {}_nat_str_.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_.bam --java-mem-size=".$memorylimit_div_mb."m &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_.bam -gtf ".$gtf." -pe -outdir \$PWD/rnaseqqc_results/{}_nat_str_.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_.bam -o {}_nat_str_.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log && samtools flagstat -@ ".$threads." {} > {}_nat_str_.bam.flagstat && && samtools stats -@ ".$threads." {} > {}_nat_str_.bam.stats' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
					}
				}
				else{
					print STDERR "ERROR:: You have requested a Paired-End analysis, so for the file $file a $mate_file file is needed\n";
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && parallel --verbose --joblog ".$projectdir."/hisat2_log_parallel.txt -j ".$parallelnumber." 'echo {} && hisat2 -q -t --seed 123 --very-sensitive ".$hisatpardef." -x ".$hisat2idx_final." -U \$(cat ".$tmp_file." | xargs dirname | uniq)/{} --met-file {}.metrics --un-conc-gz {}_no_aligned.fastq.gz | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_.bam##idx##{}_nat_str_.bam.bai - && echo {}_nat_str_.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_.bam --java-mem-size=".$memorylimit_div."m &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_.bam -gtf ".$gtf." -outdir \$PWD/rnaseqqc_results/{}_nat_str_.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_.bam -o {}_nat_str_.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log' ::: \$(cat ".$tmp_file." | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
		}
...
sub star{
	...
	if ($file and $projectdir and $logfile and $statsfile){ 
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
			...
			if($file =~ /.*_R?1\.f.*/){
				...
				if(-e $mate_file){
					if($file ne $mate_file){
						...
						$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && STAR --runThreadN ".$indexthreads." --genomeDir ".$staridx_final." --genomeLoad LoadAndExit --outFileNamePrefix ".$projectdir.$output_dir."genomeloading.tmp2 && rm -rf ".$projectdir.$output_dir."genomeloading.tmp* && parallel --verbose --joblog ".$projectdir."/star_log_parallel.txt -j ".$parallelnumber." 'echo {} && STAR --runMode alignReads --genomeDir ".$staridx_final." --genomeLoad LoadAndKeep --readFilesIn \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_1.fastq.gz \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_2.fastq.gz --outFileNamePrefix ".$projectdir.$output_dir."{}_nat_str_ $starpardef --outStd SAM | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_Aligned.out.bam##idx##{}_nat_str_Aligned.out.bam.bai - && echo {}_nat_str_Aligned.out.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_Aligned.out.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_Aligned.out.bam --java-mem-size=".$memorylimit_div."m &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_Aligned.out.bam -gtf ".$gtf." -pe -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.out.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_Aligned.out.bam -o {}_nat_str_Aligned.out.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
					}
				}
				else{
					print STDERR "ERROR:: You have requested a Paired-End analysis, so for the file $file a $mate_file file is needed\n";
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && STAR --runThreadN ".$indexthreads." --genomeDir ".$staridx_final." --genomeLoad LoadAndExit --outFileNamePrefix ".$projectdir.$output_dir."genomeloading.tmp2 && rm -rf ".$projectdir.$output_dir."genomeloading.tmp* && parallel --verbose --joblog ".$projectdir."/star_log_parallel.txt -j ".$parallelnumber." 'echo {} && STAR --runMode alignReads --genomeDir ".$staridx_final." --genomeLoad LoadAndKeep --readFilesIn \$(cat ".$tmp_file." | xargs dirname | uniq)/{} --outFileNamePrefix ".$projectdir.$output_dir."{}_nat_str_ $starpardef --outStd SAM | samtools sort -@ 3 -l 2 -m 2G --write-index -o {}_nat_str_Aligned.out.bam##idx##{}_nat_str_Aligned.out.bam.bai - && echo {}_nat_str_Aligned.out.bam && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_div_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap bamqc -bam {}_nat_str_Aligned.out.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_Aligned.out.bam --java-mem-size=".$memorylimit_div."m &>> qc.log || true && if [[ \"".$gtf."\" == *.gtf ]]; then qualimap rnaseq -bam {}_nat_str_Aligned.out.bam -gtf ".$gtf." -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.out.bam &>> qc.log || true; fi && bamCoverage -b {}_nat_str_Aligned.out.bam -o {}_nat_str_Aligned.out.bam.bw -of bigwig -bs 10 -p ".$threads." --normalizeUsing RPKM &>> bamCoverage.log' ::: \$(cat ".$tmp_file." | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log || true && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
		}
