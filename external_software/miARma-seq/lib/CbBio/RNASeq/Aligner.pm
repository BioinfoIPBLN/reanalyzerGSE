...
sub hisat2_index{
	...
	#Checking the mandatory arguments
	if ($fasta and $dir and $indexname and $logfile){
		print STDERR date()." Generating the index genome $indexname from $fasta. This process could take some hours and need much RAM memory, particularly when multithreading\n";
		#bowtie-build execution command from a fasta file. The output index will be saved
		#in the genomeindex1 directory with the name index 
	...
	if ($file and $projectdir and $hisat2idx_final and $logfile and $statsfile){ 
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
			...
			if($file =~ /.*_R?1.*/){
				...
				if(-e $mate_file){
					if($file ne $mate_file){						
						$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && parallel --verbose --joblog ".$projectdir."/hisat2_log_parallel.txt -j ".$parallelnumber." 'hisat2 -q -t --seed 123 --very-sensitive ".$hisatpardef." -x ".$hisat2idx_final." -1 \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_1.fastq.gz -2 \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_2.fastq.gz --met-file {}.metrics --un-conc-gz {}_no_aligned.fastq.gz | samtools sort -@ ".$threads." -l 2 -m 4G --write-index -o {}_nat_str_.bam##idx##{}_nat_str_.bam.bai - && echo {}_nat_str_.bam && qualimap bamqc -bam {}_nat_str_.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_.bam --java-mem-size=".$memorylimit." &>> qc.log && qualimap rnaseq -bam {}_nat_str_.bam -gtf ".$gtf." -pe -outdir \$PWD/rnaseqqc_results/{}_nat_str_.bam &>> qc.log' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
					}
				}
				else{
					print STDERR "ERROR:: You have requested a Paired-End analysis, so for the file $file a $mate_file file is needed\n";
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && parallel --verbose --joblog ".$projectdir."/hisat2_log_parallel.txt -j ".$parallelnumber." 'hisat2 -q -t --seed 123 --very-sensitive ".$hisatpardef." -x ".$hisat2idx_final." -U \$(cat ".$tmp_file." | xargs dirname | uniq)/{} --met-file {}.metrics --un-conc-gz {}_no_aligned.fastq.gz | samtools sort -@ ".$threads." -l 2 -m 4G --write-index -o {}_nat_str_.bam##idx##{}_nat_str_.bam.bai - && echo {}_nat_str_.bam && qualimap bamqc -bam {}_nat_str_.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_.bam --java-mem-size=".$memorylimit." &>> qc.log && qualimap rnaseq -bam {}_nat_str_.bam -gtf ".$gtf." -outdir \$PWD/rnaseqqc_results/{}_nat_str_.bam &>> qc.log' ::: \$(cat ".$tmp_file." | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
		}
...
sub star_index{
	...
	if ($fasta and $dir and $gtf and $logfile){
		print STDERR date()." Generating the index genome from $fasta. This process could take some hours and need much RAM memory, particularly when multithreading\n";
		#bowtie-build execution command from a fasta file. The output index will be saved
		#in the genomeindex1 directory with the name index 
...
sub star{
	...
	if ($file and $projectdir and $logfile and $statsfile){ 
		...
		if(lc($Seqtype) eq "pairedend" or lc($Seqtype) eq "paired" or lc($Seqtype) eq "paired-end"){
			...
			if($file =~ /.*_R?1.*/){
				...
				if(-e $mate_file){
					if($file ne $mate_file){
						...
						$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && STAR --runThreadN ".$indexthreads." --genomeDir ".$staridx_final." --genomeLoad LoadAndExit --outFileNamePrefix ".$projectdir.$output_dir."genomeloading.tmp2 && rm -rf ".$projectdir.$output_dir."genomeloading.tmp* && parallel --verbose --joblog ".$projectdir."/star_log_parallel.txt -j ".$parallelnumber." 'STAR --runMode alignReads --limitBAMsortRAM ".$memorylimit." --genomeDir ".$staridx_final." --genomeLoad LoadAndKeep --readFilesIn \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_1.fastq.gz \$(cat ".$tmp_file." | xargs dirname | uniq)/{}_2.fastq.gz --outFileNamePrefix ".$projectdir.$output_dir."{}_nat_str_ $starpardef --outSAMtype BAM SortedByCoordinate --outBAMcompression 2 && samtools sort -@ ".$indexthreads." -l 2 -m 4G --write-index -o {}_nat_str_Aligned.sortedByCoord.out.bam.bam##idx##{}_nat_str_Aligned.sortedByCoord.out.bam.bam.bai {}_nat_str_Aligned.sortedByCoord.out.bam && rm {}_nat_str_Aligned.sortedByCoord.out.bam && mv {}_nat_str_Aligned.sortedByCoord.out.bam.bam {}_nat_str_Aligned.sortedByCoord.out.bam && mv {}_nat_str_Aligned.sortedByCoord.out.bam.bam.bai {}_nat_str_Aligned.sortedByCoord.out.bam.bai && echo {}_nat_str_Aligned.sortedByCoord.out.bam && qualimap bamqc -bam {}_nat_str_Aligned.sortedByCoord.out.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_Aligned.sortedByCoord.out.bam --java-mem-size=".$memorylimit." &>> qc.log && qualimap rnaseq -bam {}_nat_str_Aligned.sortedByCoord.out.bam -gtf ".$gtf." -pe -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.sortedByCoord.out.bam &>> qc.log' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
					}
				}
				else{
					print STDERR "ERROR:: You have requested a Paired-End analysis, so for the file $file a $mate_file file is needed\n";
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then mkdir -p ".$projectdir.$output_dir." && cd ".$projectdir.$output_dir." && unset DISPLAY && export _JAVA_OPTIONS=\"-Xms5g -Xmx".$memorylimit_in_mb."m -Djava.io.tmpdir=\$PWD\" && STAR --runThreadN ".$indexthreads." --genomeDir ".$staridx_final." --genomeLoad LoadAndExit --outFileNamePrefix ".$projectdir.$output_dir."genomeloading.tmp2 && rm -rf ".$projectdir.$output_dir."genomeloading.tmp* && parallel --verbose --joblog ".$projectdir."/star_log_parallel.txt -j ".$parallelnumber." 'STAR --runMode alignReads --limitBAMsortRAM ".$memorylimit." --genomeDir ".$staridx_final." --genomeLoad LoadAndKeep --readFilesIn \$(cat ".$tmp_file." | xargs dirname | uniq)/{} --outFileNamePrefix ".$projectdir.$output_dir."{}_nat_str_ $starpardef --outSAMtype BAM SortedByCoordinate --outBAMcompression 2 && samtools sort -@ ".$threads." -l 2 -m 4G --write-index -o {}_nat_str_Aligned.sortedByCoord.out.bam.bam##idx##{}_nat_str_Aligned.sortedByCoord.out.bam.bam.bai {}_nat_str_Aligned.sortedByCoord.out.bam && rm {}_nat_str_Aligned.sortedByCoord.out.bam && mv {}_nat_str_Aligned.sortedByCoord.out.bam.bam {}_nat_str_Aligned.sortedByCoord.out.bam && mv {}_nat_str_Aligned.sortedByCoord.out.bam.bam.bai {}_nat_str_Aligned.sortedByCoord.out.bam.bai && echo {}_nat_str_Aligned.sortedByCoord.out.bam && qualimap bamqc -bam {}_nat_str_Aligned.sortedByCoord.out.bam -nt ".$threads." -gff ".$gtf." -c -outdir \$PWD/bamqc_results/{}_nat_str_Aligned.sortedByCoord.out.bam --java-mem-size=".$memorylimit." &>> qc.log && qualimap rnaseq -bam {}_nat_str_Aligned.sortedByCoord.out.bam -gtf ".$gtf." -outdir \$PWD/rnaseqqc_results/{}_nat_str_Aligned.sortedByCoord.out.bam &>> qc.log' ::: \$(cat ".$tmp_file." | awk -F '/' '{print \$NF}') && cd ".$projectdir.$output_dir." && for f in \$( ls | egrep '.bam\$' ); do echo -e \"\$f\t\$PWD/bamqc_results/\$f\" >> \$PWD/bamqc_results/list_multi.txt; done && mkdir -p \$PWD/samtools_results/ && parallel --verbose -j ".$parallelnumber." 'samtools flagstat -@ ".$threads." {} > \$PWD/samtools_results/{}_flagstat.txt && samtools stats -@ ".$threads." {} > \$PWD/samtools_results/{}_stats.txt' ::: \$( ls | egrep '.bam\$' ) && qualimap multi-bamqc -d \$PWD/bamqc_results/list_multi.txt -outdir \$PWD/multibamqc_results/ &>> qc.log && multiBamSummary bins -p ".$indexthreads." --bamfiles \$( ls | egrep '.bam\$' ) -out deeptools_all_bams.npz && plotCorrelation --corData deeptools_all_bams.npz --corMethod spearman --plotFile deeptools_all_bams.npz_correlation_spearman.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Spearman Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_spearman_scores.tab --plotNumbers && plotCorrelation --corData deeptools_all_bams.npz --corMethod pearson --plotFile deeptools_all_bams.npz_correlation_pearson.pdf --whatToPlot heatmap --skipZeros --plotTitle \"Pearson Correlation\" --outFileCorMatrix deeptools_all_bams.npz_correlation_pearson_scores.tab --plotNumbers && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA.pdf --outFileNameData deeptools_all_bams.npz_PCA.tab && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_2.pdf --outFileNameData deeptools_all_bams.npz_PCA_2.tab --rowCenter && plotPCA --corData deeptools_all_bams.npz --plotFile deeptools_all_bams.npz_PCA_3.pdf --outFileNameData deeptools_all_bams.npz_PCA_3.tab --transpose; fi";
		}
