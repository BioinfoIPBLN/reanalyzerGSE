...
sub featureCount{
	...
	if ($file and $projectdir and $database and $logfile){
		...
		if($file =~ /.*\.bam$/ or $file =~ /.*\.sam$/){
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then parallel --verbose --joblog ".$projectdir."/featureCounts_log_parallel.txt -j ".$parallelnumber." 'featureCounts ".$htseqpardef." -a ".$database." -o ".$projectdir.$output_dir."{}_1.fastq.gz_nat_str_Aligned.sortedByCoord.out.tab \$(find ".$projectdir." -type f -name \"*.bam\" | grep {})' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}'); fi && cd ".$projectdir." && mkdir -p \$PWD/../multiqc_out && if [ \$(ls \$PWD/../multiqc_out | wc -l) -eq 0 ]; then echo 'Gathering all QC reports with MultiQC' && multiqc -f \$PWD/../../ -n multiqc_report -o \$PWD/../multiqc_out -p -z; fi";
			...
			$commanddef="if [ \$(ls \$(find ".$projectdir." -type f -name '*.bam' | xargs dirname | uniq) | egrep '.bam\$' | egrep -c '_1.fastq.gz|_2.fastq.gz|_R1.fastq.gz|_R2.fastq.gz') -gt 0 ];then cd \$(find ".$projectdir." -type f -name '*.bam' | xargs dirname | uniq) && for i in \$(ls | egrep '.bam\$|.fastq.gz\$'); do mv \$i \$(echo \$i | sed 's/_1.fastq.gz//' | sed 's/_2.fastq.gz//' | sed 's/_R1.fastq.gz//' | sed 's/_R2.fastq.gz//'); done; fi && mkdir -p ".$projectdir.$output_dir." ;".$command." 2>> ".$logfile;;
