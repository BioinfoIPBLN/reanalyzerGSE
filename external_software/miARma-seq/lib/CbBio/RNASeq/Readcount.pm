...
sub featureCount{
	...
	if ($file and $projectdir and $database and $logfile){
		...
		if($file =~ /.*\.bam$/ or $file =~ /.*\.sam$/){
			...
			$commanddef="if [ \$(ls \$(find ".$projectdir." -type f -name '*.bam' | xargs dirname | uniq) | egrep '.bam\$' | egrep -c '_1.fastq.gz|_2.fastq.gz|_R1.fastq.gz|_R2.fastq.gz') -gt 0 ];then cd \$(find ".$projectdir." -type f -name '*.bam' | xargs dirname | uniq) && for i in *; do mv \$i \$(echo \$i | sed 's/_1.fastq.gz//' | sed 's/_2.fastq.gz//' | sed 's/_R1.fastq.gz//' | sed 's/_R2.fastq.gz//'); done; fi && mkdir -p ".$projectdir.$output_dir." ;".$command." 2>> ".$logfile;;
