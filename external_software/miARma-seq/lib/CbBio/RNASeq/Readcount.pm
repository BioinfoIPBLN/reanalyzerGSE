...
sub featureCount{
	...
	# JLR: Now compatible with latest featureCounts and samtools outside the outdated versions of miARma
	#if ($ENV{PATH}) {
	...
	if ($file and $projectdir and $database and $logfile){
		...
		if($file =~ /.*\.bam$/ or $file =~ /.*\.sam$/){
			...
			$command="if [ \$(ls ".$projectdir.$output_dir." | wc -l) -eq 0 ]; then parallel --verbose --joblog ".$projectdir."/featureCounts_log_parallel.txt -j ".$parallelnumber." 'featureCounts ".$htseqpardef." -a ".$database." -o ".$projectdir.$output_dir."{}_1.fastq.gz_nat_str_Aligned.sortedByCoord.out.tab \$(find -L ".$projectdir." -type f -name \"*.bam\" | grep {}_) &>> ".$projectdir.$output_dir."{}.log' ::: \$(cat ".$tmp_file." | sed 's,_1.fastq.gz*,,g' | sed 's,_2.fastq.gz*,,g' | sed 's,_R1.fastq.gz*,,g' | sed 's,_R2.fastq.gz*,,g' | sort | uniq | awk -F '/' '{print \$NF}'); fi && cd ".$projectdir." && mkdir -p \$PWD/../multiqc_out && if [ \$(ls \$PWD/../multiqc_out | wc -l) -eq 0 ]; then echo 'Gathering all QC reports with MultiQC' && multiqc -f \$PWD/../../ -n multiqc_report -o \$PWD/../multiqc_out -p -q; fi";
