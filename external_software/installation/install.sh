...
ln -sf $conda_envs_path/reanalyzerGSE_3/bin/multiBamSummary .
ln -sf $conda_envs_path/reanalyzerGSE_3/bin/plotCorrelation .
ln -sf $conda_envs_path/reanalyzerGSE_3/bin/plotPCA .
...
echo -e "\n\nInstalling the CRAN's package 'autoGO' and some dependencies, not available through conda as of yet... This manual installation implies that the version of the installed R packages are not frozen. So, not likely, but please do keep in mind that this may be a source of errors in the mid-term if newer versions of the R packages are installed. As of the moment of the writing of this wrapper script, the version of autoGO being installed was 0.9.1...\n\n"
export PATH=$conda_envs_path/reanalyzerGSE_3/bin:$PATH
$conda_envs_path/reanalyzerGSE_3/bin/Rscript -e 'suppressMessages({BiocManager::install(c("ComplexHeatmap","DESeq2","GSVA"),update=F,ask=F)});suppressMessages({install.packages("autoGO",repos="https://cloud.r-project.org",quiet=T)})' &> /dev/null # Avoid the very extensive logs being printed out, please remove suppressMessages and "&> /dev/null" to double check if installation fails... 
...
echo -e "\n\n\nALL DONE. Please remember that reanalyzerGSE needs at least the conda environment 'reanalyzerGSE' to be activated, or the path '$conda_envs_path/reanalyzerGSE/bin' added to the exported PATH variable\n\n\n"
