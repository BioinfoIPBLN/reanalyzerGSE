...
$conda_envs_path/reanalyzerGSE_3/bin/Rscript -e 'suppressMessages({install.packages(c("autoGO","GOxploreR","aPEAR", "rbioapi"),repos="https://cloud.r-project.org",quiet=T)})' &> /dev/null # Avoid the very extensive logs being printed out, please remove suppressMessages and "&> /dev/null" to double check if installation fails...
$conda_envs_path/reanalyzerGSE_3/bin/Rscript -e 'packages <- c("autoGO", "GOxploreR", "aPEAR", "rbioapi"); for (pkg in packages) { if(requireNamespace(pkg, quietly=TRUE)){cat(pkg,"is installed...\n")} else {cat("\n",pkg, "is NOT installed.\n")}}'
