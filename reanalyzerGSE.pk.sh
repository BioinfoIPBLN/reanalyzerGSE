...
### Gather the parameters, default values, exit if essential not provided...
	case $argument in
		-h*) echo "reanalyzerGSE v2.8.4 - usage: reanalyzerGSE.pk.sh [options]
		...
		-pR | -pattern_to_remove # A pattern to remove some matching samples from QC figures (by default 'none')
