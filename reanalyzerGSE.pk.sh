...
### Gather the parameters, default values, exit if essential not provided...
	case $argument in
		-h*) echo "reanalyzerGSE v2.8.8 - usage: reanalyzerGSE.pk.sh [options]
		...
		-D | -design_custom_local # Specifying here the experimental design for the local dataset (by default an interactive prompt will ask for a comma-separated list of the same length than the number of samples, if you want to avoid that manual input please provide the list in this argument. If more than one design to provide, please input comma-separated list separed by a '/', without spaces. Please avoid naming that would match the same pattern in grep)
...
if [[ $debug_step == "all" || $debug_step == "step4" ]]; then
	# If the running is resumed in this step, this variables has to be created because they would not exist
 	# The same may happen in other steps, this is WIP
	rm -rf $(find $output_folder/$name/ -type d -name "DGE")
	if [ -z "$organism" ]; then
		organism=$(cat $output_folder/$name/GEO_info/organism.txt | sed 's, ,_,g;s,_+,_,g')
	fi
	if [ -z "${!array[@]}" ]; then
